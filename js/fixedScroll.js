...
(function () {
	...
	fixedScroll.prototype = {
		init: function () {
			...
			// 限定起始位的距顶高度
			this.distance =
				this.stickBottom - this.stickTop - this.fixedElH - this.offset / 2;
			this.calcArea();
		...
		stickHandle: function () {
			if (this.winEl.scrollTop() > this.stickTop - this.offset) {
				if (
					this.winEl.scrollTop() <
					this.stickBottom - this.fixedElH - this.offset
				) {
					this.fixedEl.css({
						position: "fixed",
						top: this.offset,
		...
		resizeHeight: function (hasNewTop) {
			if (this.fixedEl.length > 0) {
				this.stickTop = hasNewTop
					? this.fixedEl.offset().top
					: this.init_stickTop;
			}
			if (this.stickEndEl.length > 0) {
				this.stickBottom = this.stickEndEl.offset().top;
		...
		hookScroll: function () {
			...
			for (var i in this.hookArea) {
				if (
					t > this.hookArea[i][0] - this.hookOffset &&
					t < this.hookArea[i][1]
				) {
					this.navStatus(i);
				} else {
